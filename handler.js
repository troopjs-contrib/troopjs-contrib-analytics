define([
  ...
  "when/when",
  "require"
], function (Component, AnalyticsError, State, from, start, finalize, when, localRequire) {
  "use strict";
  ...
  return Component.extend(
    function ($element, name, trackers) {
      var me = this;
      var _trackers;
      ...
      switch (OBJECT_TOSTRING.call(trackers)) {
        ...
        default:
          throw new AnalyticsError("'trackers' is of unsupported type");
      }
      ...
      me.on("sig/initialize", function () {
        return when.map(_trackers, function (_tracker) {
          ...
          switch (OBJECT_TOSTRING.call(_tracker)) {
            ...
            default:
              throw new AnalyticsError("'_tracker' is of unsupported type");
          }
